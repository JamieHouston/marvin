...
class StreamingAPI(object):
    ...
    def __repr__(self):
        return "%s(%s, %s, %s, %s) instance at %s" % (
            self.__class__.__name__, self.personal_api_token, self.flows, self.active, self.accept, hex(id(self)))
    ...
    def stream(self):
        if not self.connection or not self.connection.ok:
            self.connection = requests.get(self.API_URL, params=self.params, headers=self.headers, auth=self.auth,
                                           stream=True)
